    ...
    return this._cached<boolean>(
      ...
      () => {
        ...
        if (combinator === '+') {
          ...
          if (
            !previousSibling ||
            !this._matchesSimple(previousSibling, simple, context)
          ) {
            return false
          }
        ...
        if (combinator === '~') {
          ...
          while (previousSibling) {
            if (this._matchesSimple(previousSibling, simple, context)) {
              if (
                this._matchesParents(previousSibling, complex, index - 1, context)
              ) {
                return true
              }
              if (complex.simples[index - 1].combinator === '~') {
...
const textEngine: SelectorEngine = {
  matches(
    ...
    return (
      elementMatchesText(
        (evaluator as SelectorEvaluatorImpl)._cacheText,
        element,
        matcher
      ) === 'self'
    )
...
const hasTextEngine: SelectorEngine = {
  matches(
    ...
    const matcher = (elementText: ElementText) =>
      elementText.normalized.toLowerCase().includes(text)
    return matcher(
      elementText((evaluator as SelectorEvaluatorImpl)._cacheText, element)
    )
  },
}
    ...
    matches(
      ...
      const maxDistance =
        args.length && typeof args[args.length - 1] === 'number'
          ...
          : undefined
      const queryArgs =
        maxDistance === undefined ? args : args.slice(0, args.length - 1)
...
function previousSiblingInContext(
  element: Element,
  context: QueryContext
): Element | undefined {
  if (element === context.scope) {
